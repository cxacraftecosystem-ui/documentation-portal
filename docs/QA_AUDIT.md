...
| API server | Single t3.micro, 1 GiB; no autoscaling | Heavy concurrent media transcription could be slow | DB + S3 are off-box; uvicorn runs a SINGLE worker with the media queue in a separate fieldrepo-queue service (the 2-worker supervisor caused the orphaned-Prisma-engine 500s); scale up the instance if load grows. |
